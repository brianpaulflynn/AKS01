resource "azurerm_subnet" "aks_node_subnets" {
  for_each              = var.node_pool_map
  resource_group_name   = var.resource_group_name
  virtual_network_name  = var.virtual_network_name
  name                  = each.key
  address_prefixes      = each.value.node_address_prefixes
  dynamic "delegation" { # Grant cluster access to manage subnets
    # rem no delegation for default node pool!
    for_each = each.key != "aks_default_node_pool" ? ["${each.key}_delegation"] : []
    content {
      name      = "${each.key}_delegation"
      service_delegation {
        name    = "Microsoft.ContainerService/managedClusters"
        actions = ["Microsoft.Network/networkinterfaces/*"]
      }
    }
  }
} 
output "aks_node_subnet_ids" {
  value =  { for subnet_name, subnet in azurerm_subnet.aks_node_subnets : subnet_name => subnet.id }
}
resource "azurerm_subnet" "aks_pod_subnets" {
  for_each              = var.node_pool_map
  resource_group_name   = var.resource_group_name
  virtual_network_name  = var.virtual_network_name
  name                  = each.key
  address_prefixes      = each.value.pod_address_prefixes
  dynamic "delegation" { # Grant cluster access to manage subnets
    # rem no delegation for default pod pool!
    for_each = each.key != "aks_default_node_pool" ? ["${each.key}_delegation"] : []
    content {
      name      = "${each.key}_delegation"
      service_delegation {
        name    = "Microsoft.ContainerService/managedClusters"
        actions = ["Microsoft.Network/networkinterfaces/*"]
      }
    }
  }
} 
output "aks_pod_subnet_ids" {
  value =  { for subnet_name, subnet in azurerm_subnet.aks_pod_subnets : subnet_name => subnet.id }
}
# resource "azurerm_subnet" "aks_subnets" {
#   for_each              = var.aks_config.subnets_map
#   resource_group_name   = var.resource_group_name
#   virtual_network_name  = var.virtual_network_name
#   name                  = each.key
#   address_prefixes      = each.value.address_prefixes
#   dynamic "delegation" { # Grant cluster access to manage subnets
#     for_each = var.aks_config.node_pool_map.node_address_prefixes[each.key].service_delegation_name != null ? [var.aks_config.node_pool_map.node_address_prefixes[each.key].service_delegation_name] : []
#     content {
#       name      = "${var.aks_config.node_pool_map.node_address_prefixes[each.key].service_delegation_name}-delegation"
#       service_delegation {
#         name    = "Microsoft.ContainerService/managedClusters"
#         actions = ["Microsoft.Network/networkinterfaces/*"]
#       }
#     }
#   }
# } 
